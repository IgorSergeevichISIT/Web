...
**Выполнено:**
- Метод `groups.getMembers` для получения участников группы  
- Метод `users.get` для получения информации о пользователе  
- Компонент сортировки `SortFilterComponent`** с вариантами:
  - Без сортировки
  - ID по возрастанию
  - ID по убыванию
  - Дата вступления по возрастанию
  - Дата вступления по убыванию
...
**2. Модуль для работы с API**
   - Создан класс `Ajax` для работы с `XMLHttpRequest`
   - Реализованы методы `get()` и `post()`
   - Обработка ошибок сети и JSON парсинга
   - Асинхронная работа с колбэками
