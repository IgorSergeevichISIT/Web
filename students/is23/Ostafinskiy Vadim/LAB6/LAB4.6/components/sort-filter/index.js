export class SortFilterComponent {
    ...
    getSortDescription(sort) {
        const descriptions = {
            '': 'Показываются все участники в порядке получения (добавленные + VK)',
            'id_asc': 'Участники отсортированы по ID (от меньшего к большему). Добавленные пользователи с отрицательными ID показываются первыми.',
            'id_desc': 'Участники отсортированы по ID (от большего к меньшему). VK пользователи с положительными ID показываются первыми.',
            'time_asc': 'Сначала добавленные пользователи (по дате добавления), затем VK пользователи (по ID)',
            'time_desc': 'Сначала VK пользователи (по ID), затем добавленные пользователи (по дате добавления)'
        };
