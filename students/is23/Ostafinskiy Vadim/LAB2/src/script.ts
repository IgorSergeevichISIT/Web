let a: string = '';
let b: string = '';
let expressionResult: number = 0;
let selectedOperation: string | null = null;
const outputElement: HTMLElement = document.getElementById("result")!;
...
function showError(message: string = 'Error'): void {
    outputElement.innerHTML = message;
    resetCalculator();
}

function setupEventListeners(): void {
    const digitButtons: NodeListOf<Element> = document.querySelectorAll('[id ^= "btn_digit_"]');
    ...
    document.getElementById("btn_op_mult")!.addEventListener('click', () => { 
        if (a === '' || outputElement.innerHTML === 'Error' || outputElement.innerHTML === 'Division by zero!') return;
        selectedOperation = 'x';
        expressionResult = 0;
    });
    ...
    document.getElementById("btn_op_sign")!.addEventListener('click', () => {
        if (outputElement.innerHTML === 'Error' || outputElement.innerHTML === 'Division by zero!') return;
