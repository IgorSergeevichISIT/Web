...
class Calculator {
    ...
    private resultColorIndex: number = 0;
    private bgColorIndex: number = 0;
    private hasError: boolean = false; // Флаг для отслеживания состояния ошибки

    // Массивы цветов для смены
    private resultColors: string[] = ['#00ff00', '#ff00ff', '#00ffff', '#ffff00', '#ff6600'];
    private bgColors: string[] = ['#1a1a1a', '#2d2d2d', '#1a3a1a', '#3a1a1a', '#1a1a3a'];
    ...
    private changeBackgroundColor(): void {
        const wrapper = document.querySelector('.calculator-wrapper') as HTMLElement;
        if (wrapper) {
